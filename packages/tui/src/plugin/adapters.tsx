import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@clawc/plugin/tui"
...
import type { useClawcKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useClawcKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getClawcModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getClawcModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
