...
export async function installWslClawc(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
...
export async function resolveWslClawc(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
