...
import { ClawcClient, type GlobalEvent } from "@clawc/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ClawcClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ClawcClient["global"]["event"]>>["stream"]
...
type SessionChild = NonNullable<Awaited<ReturnType<ClawcClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ClawcClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: ClawcClient["event"]["subscribe"]
    globalEvent?: ClawcClient["global"]["event"]
    promptAsync?: ClawcClient["session"]["promptAsync"]
    status?: ClawcClient["session"]["status"]
    messages?: ClawcClient["session"]["messages"]
    children?: ClawcClient["session"]["children"]
    permissions?: ClawcClient["permission"]["list"]
    questions?: ClawcClient["question"]["list"]
  } = {},
) {
  const client = new ClawcClient()

  const subscribe: ClawcClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ClawcClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ClawcClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ClawcClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ClawcClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ClawcClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ClawcClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ClawcClient["question"]["list"] = input.questions ?? (() => ok([]))
