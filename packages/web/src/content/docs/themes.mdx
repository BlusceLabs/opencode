...
With ClawC you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, ClawC uses our own `clawc` theme.
...
ClawC comes with several built-in themes.
...
- Want ClawC to match their terminal's appearance
...
ClawC supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/clawc/themes/*.json` or `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.clawc/themes/*.json`
4. **Current working directory** - Defined in `./.clawc/themes/*.json`
