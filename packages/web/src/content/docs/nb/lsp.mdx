---
title: LSP-servere
description: clawc integreres med LSP-serverne dine.
---

ClawC kan integreres med Language Server Protocol (LSP)-servere for å bruke diagnostikk som tilbakemelding til agenten.

---

## Innebygde servere

ClawC kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `CLAWC_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når LSP er aktivert og clawc åpner en fil, gjør den:
...
Du kan aktivere og tilpasse LSP-servere gjennom `lsp`-delen i ClawC-konfigurasjonen.
