---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i ClawC.
---

import config from "../../../../config.mjs"
export const console = config.console

ClawC bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i ClawC-konfigurasjonen.
...
i `~/.local/share/clawc/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i ClawC
konfig.
...
ClawC Zen er en liste over modeller levert av ClawC-teamet som har vært
testet og verifisert for å fungere godt med ClawC. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med ClawC Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `ClawC Zen` og gå til [clawc.ai/auth](https://clawc.ai/zen).
...
Det fungerer som alle andre leverandører i ClawC og er helt valgfritt å bruke.

---

## ClawC Go

ClawC Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av ClawC-teamet som har vært
testet og verifisert for å fungere godt med ClawC.

1. Kjør kommandoen `/connect` i TUI, velg `ClawC Go`, og gå til [clawc.ai/auth](https://clawc.ai/zen).
...
Slik bruker du Amazon Bedrock med ClawC:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører ClawC:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `clawc.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i clawc støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere clawc til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at ClawC skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom ClawC-konfigurasjonen.
...
for å lagre token i ClawC auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
ClawC bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse ClawC
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`clawc.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `clawc.json`:
...
Slik bruker du GitHub Copilot-abonnementet med ClawC:
...
Slik bruker du Google Vertex AI med ClawC:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom ClawC, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session) plugin for automatisk å logge hver ClawC-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver ClawC-samtale oppført som en separat økt.
...
Du kan konfigurere ClawC for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere ClawC for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere ClawC for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for ClawC. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/clawc) for detaljer.
:::
...
Slik bruker du Ollama Cloud med ClawC:
...
3. Kopier API-nøkkelen for bruk i ClawC.
...
6. **Viktig**: Før du bruker skymodeller i ClawC, må du hente modellinformasjonen lokalt:
...
ClawC Zen er en liste over testede og verifiserte modeller levert av ClawC-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>ClawC Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **ClawC Zen**.

   ```txt
   /connect
   ```

3. Skriv inn ClawC API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom ClawC-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom ClawC-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med ClawC:
...
Du kan også tilpasse modeller gjennom ClawC-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med ClawC. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `clawc.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar ClawC forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `clawc auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk ClawC-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i ClawC-konfigurasjonen.
