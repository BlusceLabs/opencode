...
ClawC Go er et lavkostnadsabonnement — **$5 for din første måned**, deretter **$10/måned** — som gir deg pålitelig tilgang til populære åpne kodemodeller.

Go fungerer som enhver annen leverandør i ClawC. Du abonnerer på ClawC Go og
...
ClawC Go gir deg tilgang til disse modellene for **$5 for din første måned**, deretter **$10/måned**.

---

## Hvordan det fungerer

ClawC Go fungerer som enhver annen leverandør i ClawC.

1. Du logger inn på **<a href={console}>ClawC Zen</a>**, abonnerer på Go, og
   kopierer din API-nøkkel.
2. Du kjører kommandoen `/connect` i TUI-en, velger `ClawC Go`, og limer inn
   din API-nøkkel.
3. Kjør `/models` i TUI-en for å se listen over modeller som er tilgjengelige gjennom Go.

:::note
Kun ett medlem per arbeidsområde kan abonnere på ClawC Go.
:::
...
ClawC Go inkluderer følgende grenser:
...
[Modell-ID-en](/docs/config/#models) i din ClawC-konfigurasjon
bruker formatet `clawc-go/<model-id>`. For eksempel, for Kimi K2.7 Code, vil du
bruke `clawc-go/kimi-k2.7-code` i konfigurasjonen din.
...
4. Ha **ingen innelåsing** ved å la deg bruke en hvilken som helst annen leverandør med ClawC også.
