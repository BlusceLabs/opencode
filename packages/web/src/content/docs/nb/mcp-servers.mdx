...
Du kan legge til eksterne verktøy i ClawC ved å bruke _Model Context Protocol_ eller MCP. ClawC støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [ClawC Config](https://clawc.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="clawc.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/clawc`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
ClawC håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil ClawC:
...
Hvis serveren krever autentisering, vil ClawC be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `clawc mcp auth <server-name>`.
...
```bash
clawc mcp auth my-oauth-server
```
...
```bash
clawc mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil ClawC lagre tokens sikkert i `~/.local/share/clawc/mcp-auth.json`.
...
clawc mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i ClawC, sammen med innebygde verktøy. Så du kan administrere dem gjennom ClawC-konfigurasjonen som alle andre verktøy.
...
```json title="clawc.json" {11, 14-18}
{
  "$schema": "https://clawc.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
clawc mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble ClawC til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver ClawC-samtale oppført som en separat økt.
