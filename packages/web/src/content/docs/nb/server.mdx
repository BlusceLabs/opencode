---
title: Tjener
description: Samhandle med ClawC-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `clawc serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en ClawC-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `clawc.local` |
...
Sett `CLAWC_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `clawc` som standard, eller sett `CLAWC_SERVER_USERNAME` for å overstyre det. Dette gjelder både `clawc serve` og `clawc web`.
...
Når du kjører `clawc` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk ClawC-serveren til å samhandle med ClawC programmatisk.
:::

Denne arkitekturen lar ClawC støtte flere klienter og lar deg samhandle med ClawC programmatisk.

Du kan kjøre `clawc serve` for å starte en frittstående server. Hvis du har
ClawC TUI kjører, vil `clawc serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av ClawC [IDE](/docs/ide) plugins.
...
clawc-serveren viser følgende APIer.
