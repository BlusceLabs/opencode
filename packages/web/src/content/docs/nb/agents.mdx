...
Det er to typer agenter i ClawC; primære agenter og underagenter.
...
ClawC kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
ClawC kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

ClawC kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i ClawC sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `clawc.json`:
...
- Globalt: `~/.config/clawc/agents/`
- Per prosjekt: `.clawc/agents/`
...
Hvis ingen temperatur er spesifisert, bruker ClawC modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale ClawC-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i ClawC-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [ClawC Zen](/docs/zen), vil du bruke `clawc/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `clawc models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/clawc).
:::
