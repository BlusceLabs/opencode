---
title: サーバー
description: HTTP 経由で ClawC サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`clawc serve` コマンドは、ClawC クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `clawc.local` |
...
HTTP 基本認証でサーバーを保護するには、`CLAWC_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `clawc` になるか、`CLAWC_SERVER_USERNAME` を設定してオーバーライドします。これは、`clawc serve` と `clawc web` の両方に当てはまります。
...
`clawc` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
ClawC サーバーを使用して、プログラムで ClawC と対話します。
:::

`clawc serve` を実行してスタンドアロンサーバーを起動できます。すでに ClawC TUI を実行している場合でも、`clawc serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、ClawC [IDE](/docs/ide) プラグインによって使用されます。
