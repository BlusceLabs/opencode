---
title: Plugins
description: Écrivez vos propres plugins pour étendre ClawC.
---

Les plugins vous permettent d'étendre ClawC en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de ClawC.
...
- `.clawc/plugins/` - Plugins au niveau du projet
- `~/.config/clawc/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/clawc/node_modules/`.
...
1. Configuration globale (`~/.config/clawc/clawc.json`)
2. Configuration du projet (`clawc.json`)
3. Répertoire global des plugins (`~/.config/clawc/plugins/`)
4. Répertoire des plugins du projet (`.clawc/plugins/`)
...
ClawC exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK clawc pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre clawc.
...
:::note
Si vous utilisez l'application de bureau ClawC, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez clawc de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à clawc :
...
L'assistant `tool` crée un outil personnalisé que clawc peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour clawc aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://clawc.ai/docs/sdk) pour plus de détails.
