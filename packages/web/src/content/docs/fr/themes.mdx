...
Avec ClawC, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, ClawC utilise notre propre thème `clawc`.
...
ClawC est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que ClawC corresponde à l'apparence de leur terminal
...
ClawC prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/clawc/themes/*.json` ou `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.clawc/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.clawc/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/clawc/themes
vim ~/.config/clawc/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .clawc/themes
vim .clawc/themes/my-theme.json
```
...
{
  "$schema": "https://clawc.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
