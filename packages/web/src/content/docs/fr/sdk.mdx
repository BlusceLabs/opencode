---
title: SDK
description: Client JS de type sécurisé pour le serveur clawc.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK clawc JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler clawc par programme.
...
Créez une instance de clawc :

```javascript
import { createClawc } from "@clawc/sdk"

const { client } = await createClawc()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `clawc.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${clawc.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de clawc, vous pouvez créer une instance client pour vous y connecter :
