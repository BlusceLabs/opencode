...
Mit ClawC kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet ClawC das `clawc`-Theme.
...
ClawC liefert mehrere eingebaute Themes mit.
...
- ClawC optisch ans Terminal anpassen wollen
...
ClawC unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/clawc/themes/*.json` oder `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.clawc/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.clawc/themes/*.json`
