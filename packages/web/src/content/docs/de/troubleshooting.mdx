...
Wenn ClawC Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\clawc\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `clawc --log-level DEBUG`.

---

## Speicher

clawc speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/clawc/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\clawc`
...
ClawC Desktop startet im Hintergrund einen lokalen ClawC-Server (`clawc-cli`-Sidecar).
...
- Nur macOS: `ClawC`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/clawc/clawc.jsonc` (oder `~/.config/clawc/clawc.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/clawc/clawc.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\clawc\clawc.jsonc`
...
ClawC kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/clawc/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\clawc\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit ClawC ihn neu aufbauen kann.

1. Beende ClawC Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/clawc`
- **Linux**: loesche `~/.cache/clawc` (oder fuehre aus: `rm -rf ~/.cache/clawc`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\clawc`

3. Starte ClawC Desktop neu.

---

### Server-Verbindungsprobleme beheben

ClawC Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `clawc.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CLAWC_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CLAWC_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt ClawC Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit ClawC-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

ClawC Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer ClawC in den OS-Einstellungen aktiviert sind, und
...
1. Beende ClawC Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von ClawC Desktop):

- `clawc.settings.dat` (desktop default server URL)
- `clawc.global.dat` und `clawc.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit ClawC hast:
...
3. Stelle sicher, dass du die neueste Version hast: `clawc upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `clawc models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\clawc`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. clawc installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\clawc`

2. Starte clawc neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt clawc, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
clawc erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
