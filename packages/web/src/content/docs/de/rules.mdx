---
title: Regeln
description: Lege eigene Anweisungen fuer clawc fest.
---

Du kannst clawc ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in clawc den Befehl `/init` aus.
...
So kann clawc sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht clawc sie zu erweitern.
...
clawc kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/clawc/AGENTS.md` speichern.
Diese gelten in allen clawc-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt ClawC dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/clawc/AGENTS.md` existiert)
...
Beim Start sucht clawc in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/clawc/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `clawc.json` oder in `~/.config/clawc/clawc.json` angeben.
...
clawc parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit clawc.json

Empfohlen ist das `instructions`-Feld in der `clawc.json`:
...
Du kannst clawc explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `clawc.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
