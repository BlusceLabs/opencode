...
Agent Skills erlauben ClawC, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
ClawC sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft ClawC vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.clawc/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.clawc/skills/git-release/SKILL.md` zum Beispiel so:
...
ClawC listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `clawc.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `clawc.json`):
