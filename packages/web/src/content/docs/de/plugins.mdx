---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um ClawC zu erweitern.
---

Mit Plugins können Sie ClawC erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von ClawC zu ändern.
...
- `.clawc/plugins/` – Plugins auf Projektebene
- `~/.config/clawc/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/clawc/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/clawc/clawc.json`)
2. Projektkonfiguration (`clawc.json`)
3. Globales Plugin-Verzeichnis (`~/.config/clawc/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.clawc/plugins/`)
...
ClawC führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein ClawC-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie ClawC erweitern können.
...
:::note
Wenn Sie die ClawC-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass ClawC `.env`-Dateien liest:
...
Plugins können ClawC auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das ClawC aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für ClawC zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://clawc.ai/docs/sdk).
