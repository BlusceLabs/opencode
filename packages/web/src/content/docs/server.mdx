---
title: Server
description: Interact with clawc server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `clawc serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an clawc client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `clawc.local` |
...
Set `CLAWC_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `clawc`, or set `CLAWC_SERVER_USERNAME` to override it. This applies to both `clawc serve` and `clawc web`.
...
When you run `clawc` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the clawc server to interact with clawc programmatically.
:::

This architecture lets clawc support multiple clients and allows you to interact with clawc programmatically.

You can run `clawc serve` to start a standalone server. If you have the
clawc TUI running, `clawc serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the ClawC [IDE](/docs/ide) plugins.
...
The clawc server exposes the following APIs.
