...
You can add external tools to ClawC using the _Model Context Protocol_, or MCP. ClawC supports both local and remote servers.
...
You can define MCP servers in your [ClawC Config](https://clawc.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/clawc` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
ClawC automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, ClawC will:
...
If the server requires authentication, ClawC will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `clawc mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, ClawC will store the tokens securely in `~/.local/share/clawc/mcp-auth.json`.
...
Your MCPs are available as tools in ClawC, alongside built-in tools. So you can manage them through the ClawC config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect ClawC to your Sentry account.
