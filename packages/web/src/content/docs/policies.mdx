---
title: Policies
description: Control which configured resources ClawC may use.
---

Policies control whether ClawC may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `clawc.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether ClawC may use a resource such as an LLM provider.
...
ClawC currently supports one policy action:
