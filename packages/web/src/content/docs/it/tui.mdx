---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di ClawC.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ClawC fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire ClawC avvia la TUI per la directory corrente.
...
Quando usi la TUI di ClawC, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a ClawC. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `clawc.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo ClawC attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `clawc.default`.
