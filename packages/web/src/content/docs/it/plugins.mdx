---
title: Plugin
description: Scrivi plugin per estendere ClawC.
---

I plugin ti permettono di estendere ClawC agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di ClawC.
...
- `.clawc/plugins/` - plugin a livello progetto
- `~/.config/clawc/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/clawc/node_modules/`.
...
1. Config globale (`~/.config/clawc/clawc.json`)
2. Config di progetto (`clawc.json`)
3. Directory plugin globale (`~/.config/clawc/plugins/`)
4. Directory plugin di progetto (`.clawc/plugins/`)
...
ClawC esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di clawc per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere clawc.
...
:::note
Se usi l'app desktop di ClawC, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a clawc di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a clawc:
...
L'helper `tool` crea uno strumento personalizzato che clawc può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in clawc insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://clawc.ai/docs/sdk) per i dettagli.
