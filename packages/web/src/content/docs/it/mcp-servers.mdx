...
Puoi aggiungere strumenti esterni a ClawC usando il _Model Context Protocol_, o MCP. ClawC supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [ClawC Config](https://clawc.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/clawc`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
ClawC gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, ClawC:
...
Se il server richiede autenticazione, ClawC ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `clawc mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, ClawC memorizzera' i token in modo sicuro in `~/.local/share/clawc/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in ClawC insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione ClawC come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare ClawC al tuo account Sentry.
