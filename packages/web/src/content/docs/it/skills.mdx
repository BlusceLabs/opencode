...
Le skill degli agenti permettono a ClawC di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
ClawC cerca in queste posizioni:

- Config di progetto: `.clawc/skills/<name>/SKILL.md`
- Config globale: `~/.config/clawc/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, ClawC risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.clawc/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.clawc/skills/git-release/SKILL.md` cosi':
...
ClawC elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `clawc.json`:
...
**Per agenti integrati** (in `clawc.json`):
