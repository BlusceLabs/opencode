---
title: Server
description: Interagisci con il server clawc via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `clawc serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client clawc.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `clawc.local` |
...
Imposta `CLAWC_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `clawc`, oppure imposta `CLAWC_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `clawc serve` sia per `clawc web`.
...
Quando esegui `clawc` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server clawc per interagire con clawc in modo programmatico.
:::

Questa architettura permette a clawc di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `clawc serve` per avviare un server standalone. Se la TUI di clawc e gia in esecuzione, `clawc serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di ClawC.
...
Il server clawc espone le seguenti API.
