...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. ClawC posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w ClawC.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (lub `CLAWC_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy ClawC lub gdy zmienna środowiskowa `CLAWC_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu ClawC:
