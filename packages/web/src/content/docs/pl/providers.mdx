---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w clawc.
---

import config from "../../../../config.mjs"
export const console = config.console

clawc używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji clawc.
...
w `~/.local/share/clawc/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim clawc
config.
...
ClawC Zen to lista modeli dostarczonych przez zespół clawc, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z clawc. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od ClawC Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `ClawC Zen` i przejdź do [clawc.ai/auth](https://clawc.ai/zen).
...
Działa jak każdy inny dostawca w clawc i jest całkowicie opcjonalny w użyciu.

---

## ClawC Go

ClawC Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół clawc, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z clawc.

1. Uruchom polecenie `/connect` w TUI, wybierz `ClawC Go` i przejdź do [clawc.ai/auth](https://clawc.ai/zen).
...
Aby używać Amazon Bedrock z clawc:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania clawc:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `clawc.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w clawc nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować clawc do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod clawc działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji clawc.
...
clawc używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować clawc
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`clawc.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem clawc:
...
Aby używać Google Vertex AI z clawc:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania clawc.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą clawc, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session), aby automatycznie rejestrować każdą konwersację clawc jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja clawc będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować clawc tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować clawc tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować clawc tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem clawc. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/clawc).
:::
...
Aby korzystać z Ollama Cloud z clawc:
...
3. Skopiuj klucz API do użycia w clawc.
...
6. **Ważne**: Przed użyciem modeli chmurowych w clawc musisz pobrać informacje o modelu lokalnie:
...
ClawC Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół clawc. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>ClawC Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **ClawC Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API clawc.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji clawc.
...
5. Możesz je także dostosować za pomocą konfiguracji clawc. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z clawc:
...
Możesz także dostosować modele za pomocą konfiguracji clawc. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z clawc. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `clawc.json` w katalogu projektu:
...
Pola `limit` pozwalają clawc zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `clawc auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację clawc i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji clawc.
