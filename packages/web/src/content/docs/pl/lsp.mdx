---
title: Serwery LSP
description: ClawC integruje się z serwerami LSP.
---

ClawC może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

ClawC posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `CLAWC_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i ClawC otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji ClawC.
