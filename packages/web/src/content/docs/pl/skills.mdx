...
Umiejętności agenta pozwalają clawc odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
clawc przeszukuje te lokalizacje:

- Project config: `.clawc/skills/<name>/SKILL.md`
- Global config: `~/.config/clawc/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu clawc przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.clawc/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.clawc/skills/git-release/SKILL.md` w ten sposób:
...
clawc wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `clawc.json`:
...
**Dla agentów wbudowanych** (w `clawc.json`):
