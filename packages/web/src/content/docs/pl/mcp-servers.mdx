...
Możesz dodać dodatkowe informacje do clawc za pomocą _Model Context Protocol_, w skrócie MCP. clawc obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [clawc Config](https://clawc.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/clawc`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
clawc automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, clawc:
...
Jeśli serwer wymaga uwierzytelnienia, clawc poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `clawc mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji clawc bezpieczne przechowa tokeny w `~/.local/share/clawc/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w clawc, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie clawc, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć clawc z kontem Sentry.
