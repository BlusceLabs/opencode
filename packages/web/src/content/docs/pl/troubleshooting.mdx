...
Aby debugować problemy z clawc, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\clawc\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `clawc --log-level DEBUG`.

---

## Przechowywanie danych

clawc przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/clawc/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\clawc`
...
clawc Desktop uruchamia w tle lokalny serwer clawc (sidecar `clawc-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `ClawC` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/clawc/clawc.jsonc` (lub `~/.config/clawc/clawc.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/clawc/clawc.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\clawc\clawc.jsonc`
...
clawc może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/clawc/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\clawc\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby clawc mógł ją odbudować.

1. Całkowicie zamknij clawc Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/clawc`
- **Linux**: usuń `~/.cache/clawc` (lub uruchom `rm -rf ~/.cache/clawc`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\clawc`

3. Uruchom ponownie clawc Desktop.

---

### Napraw problemy z połączeniem z serwerem

clawc Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `clawc.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CLAWC_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CLAWC_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows clawc Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami clawc.

---

### Brak powiadomień

clawc Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla clawc w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie clawc Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji clawc Desktop):

- `clawc.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `clawc.global.dat` i `clawc.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `clawc upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `clawc models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\clawc`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. clawc dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\clawc`

2. Uruchom ponownie kod clawc, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to clawc do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
clawc wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
