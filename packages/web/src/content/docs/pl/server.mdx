---
title: Serwer
description: Interakcja z serwerem clawc za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `clawc serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient clawc.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `clawc.local` |
...
Ustaw `CLAWC_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `clawc` lub ustaw `CLAWC_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `clawc serve`, jak i `clawc web`.
...
Kiedy uruchomisz `clawc`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera clawc do programowej interakcji z clawc.
:::

Ta architektura umożliwia obsługę wielu klientów przez clawc i programową interakcję z kodem otwartym.

Możesz uruchomić `clawc serve`, aby uruchomić samodzielny serwer. Jeśli masz
clawc TUI działa, `clawc serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki clawc [IDE](/docs/ide).
...
Serwer clawc udostępnia następujące interfejsy API.
