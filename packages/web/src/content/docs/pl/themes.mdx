...
Dzięki clawc możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie clawc używa naszego własnego motywu `clawc`.
...
clawc ma kilka wbudowanych motywów.
...
- Chcą, aby ClawC pasował do wyglądu ich terminala
...
clawc obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/clawc/themes/*.json` lub `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.clawc/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.clawc/themes/*.json`
