---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć clawc.
---

Wtyczki umożliwiające wyłączenie clawc poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania clawc.
...
- `.clawc/plugins/` - Wtyczki na poziomie projektu
- `~/.config/clawc/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/clawc/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/clawc/clawc.json`)
2. Konfiguracja projektu (`clawc.json`)
3. Globalny katalog wtyczek (`~/.config/clawc/plugins/`)
4. Katalog wtyczek projektu (`.clawc/plugins/`)
...
clawc uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z clawc do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do clawc.
...
:::note
Jeśli korzystasz z aplikacji komputerowej clawc, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj clawc czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do clawc:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod clawc. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://clawc.ai/docs/sdk).
