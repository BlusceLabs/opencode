...
مع ClawC يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم ClawC سمتنا `clawc`.
...
- يريدون أن يطابق ClawC مظهر terminal لديهم
...
يدعم ClawC نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/clawc/themes/*.json` أو `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.clawc/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.clawc/themes/*.json`
