...
أنشئ الملف `.clawc/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات ClawC أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ ClawC:
...
- على مستوى النظام: `~/.config/clawc/commands/`
- لكل مشروع: `.clawc/commands/`
...
يتضمن clawc عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
