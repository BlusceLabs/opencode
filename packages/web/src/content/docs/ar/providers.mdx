---
title: المزوّدون
description: استخدام أي موفّر LLM في ClawC.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم ClawC ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات ClawC.
...
في `~/.local/share/clawc/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات ClawC.
...
ClawC Zen هي قائمة نماذج يوفّرها فريق ClawC وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع ClawC. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع ClawC Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر clawc، ثم انتقل إلى [clawc.ai/auth](https://clawc.ai/auth).
...
يعمل مثل أي مزوّد آخر في ClawC واستخدامه اختياري بالكامل.

---

## ClawC Go

ClawC Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق ClawC، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع ClawC.

1. شغّل الأمر `/connect` في TUI، واختر `ClawC Go`، ثم انتقل إلى [clawc.ai/auth](https://clawc.ai/zen).
...
لاستخدام Amazon Bedrock مع ClawC:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل clawc:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `clawc.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل ClawC غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين clawc لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل clawc بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات clawc.
...
تخزين الرمز في مساحة تخزين مصادقة clawc.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم ClawC نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر ClawC
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`clawc.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع clawc:
...
لاستخدام Google Vertex AI مع ClawC:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر clawc، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session) لتسجيل كل محادثة ClawC تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة ClawC مدرجة كجلسة مستقلة.
...
يمكنك ضبط clawc لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط clawc لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط clawc لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع ClawC. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/clawc) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع ClawC:
...
3. انسخ مفتاح API لاستخدامه في ClawC.
...
6. **مهم**: قبل استخدام نماذج السحابة في ClawC، يجب سحب معلومات النموذج محليا:
...
ClawC Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق ClawC. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>ClawC Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **ClawC Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ ClawC.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات clawc.
...
5. يمكنك أيضا تخصيصها عبر إعدادات clawc. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع ClawC:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات clawc. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع clawc. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `clawc.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ ClawC بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `clawc auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات clawc وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات clawc.
