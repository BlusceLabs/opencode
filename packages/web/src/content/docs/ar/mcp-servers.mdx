...
يمكنك إضافة أدوات خارجية إلى ClawC باستخدام _Model Context Protocol_، أو MCP. يدعم ClawC خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات ClawC](https://clawc.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/clawc` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى ClawC تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم ClawC بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك ClawC المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `clawc mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن ClawC الرموز بشكل آمن في `~/.local/share/clawc/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل ClawC إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات ClawC مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط ClawC بحساب Sentry الخاص بك.
