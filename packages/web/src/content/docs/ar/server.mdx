---
title: الخادم
description: تفاعل مع خادم clawc عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `clawc serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل clawc استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `clawc.local` |
...
عيّن `CLAWC_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `clawc`، أو عيّن `CLAWC_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `clawc serve` و `clawc web`.
...
عند تشغيل `clawc` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم clawc للتفاعل مع clawc برمجيا.
:::

تتيح هذه البنية لـ clawc دعم عدة عملاء وتمكّنك من التفاعل مع clawc برمجيا.

يمكنك تشغيل `clawc serve` لبدء خادم مستقل. إذا كانت واجهة clawc في terminal (TUI)
قيد التشغيل، فسيبدأ `clawc serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات ClawC لـ [IDE](/docs/ide).
...
يعرض خادم clawc واجهات API التالية.
