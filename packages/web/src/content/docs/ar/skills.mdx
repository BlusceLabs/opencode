...
تتيح مهارات الوكيل لـ ClawC اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث ClawC في هذه المواقع:

- إعدادات المشروع: `.clawc/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/clawc/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج ClawC صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.clawc/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/clawc/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.clawc/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض ClawC المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `clawc.json`:
...
**للوكلاء المدمجين** (في `clawc.json`):
