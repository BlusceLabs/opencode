...
Existem dois tipos de agentes no clawc; agentes primários e subagentes.
...
clawc vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
ClawC vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

ClawC vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do ClawC, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `clawc.json`:
...
- Global: `~/.config/clawc/agents/`
- Por projeto: `.clawc/agents/`

```markdown title="~/.config/clawc/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o clawc usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do clawc quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do clawc usa o formato `provider/model-id`. Por exemplo, se você estiver usando [ClawC Zen](/docs/zen), você usaria `clawc/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="clawc.json" {3-6,9-12}
{
  "$schema": "https://clawc.ai/config.json",
  "tools": {
    "write": true,
...
```json title="clawc.json" {3-5,8-10}
{
  "$schema": "https://clawc.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `clawc models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/clawc).
:::
...
```markdown title="~/.config/clawc/agents/docs-writer.md"
---
...
```markdown title="~/.config/clawc/agents/security-auditor.md"
---
