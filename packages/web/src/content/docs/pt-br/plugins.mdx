---
title: Plugins
description: Escreva seus próprios plugins para estender o clawc.
---

Plugins permitem que você estenda o clawc conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do clawc.
...
- `.clawc/plugins/` - Plugins em nível de projeto
- `~/.config/clawc/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/clawc/node_modules/`.
...
1. Configuração global (`~/.config/clawc/clawc.json`)
2. Configuração do projeto (`clawc.json`)
3. Diretório de plugins global (`~/.config/clawc/plugins/`)
4. Diretório de plugins do projeto (`.clawc/plugins/`)
...
O clawc executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do clawc para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o clawc.
...
:::note
Se você estiver usando o aplicativo desktop clawc, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o clawc de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao clawc:
...
O helper `tool` cria uma ferramenta personalizada que o clawc pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o clawc junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://clawc.ai/docs/sdk) para detalhes.
