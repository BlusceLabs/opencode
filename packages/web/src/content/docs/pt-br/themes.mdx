...
Com o clawc, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o clawc usa nosso próprio tema `clawc`.
...
O clawc vem com vários temas embutidos.
...
- Querem que o clawc corresponda à aparência do seu terminal
...
O clawc suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/clawc/themes/*.json` ou `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.clawc/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.clawc/themes/*.json`
