...
Para depurar problemas com o clawc, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\clawc\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `clawc --log-level DEBUG`.

---

## Armazenamento

O clawc armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/clawc/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\clawc`
...
O clawc Desktop executa um servidor clawc local (o sidecar `clawc-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `ClawC` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/clawc/clawc.jsonc` (ou `~/.config/clawc/clawc.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/clawc/clawc.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\clawc\clawc.jsonc`
...
O clawc também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/clawc/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\clawc\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o clawc possa reconstruí-lo.

1. Saia completamente do clawc Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/clawc`
- **Linux**: exclua `~/.cache/clawc` (ou execute `rm -rf ~/.cache/clawc`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\clawc`

3. Reinicie o clawc Desktop.

---

### Corrigindo problemas de conexão com o servidor

O clawc Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `clawc.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `CLAWC_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `CLAWC_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o clawc Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do clawc.

---

### Notificações não aparecendo

O clawc Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o clawc nas configurações do seu sistema operacional, e
...
1. Saia do clawc Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo clawc Desktop):

- `clawc.settings.dat` (URL do servidor padrão do desktop)
- `clawc.global.dat` e `clawc.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o clawc:
...
3. Certifique-se de que você tem a versão mais recente com `clawc upgrade`
...
Para descobrir quais modelos você tem acesso, execute `clawc models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\clawc`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O clawc instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\clawc`

2. Reinicie o clawc para reinstalar os pacotes de provedores mais recentes

Isso forçará o clawc a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O clawc detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
