...
As habilidades do agente permitem que o clawc descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O clawc pesquisa nesses locais:

- Configuração do projeto: `.clawc/skills/<name>/SKILL.md`
- Configuração global: `~/.config/clawc/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o clawc sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.clawc/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.clawc/skills/git-release/SKILL.md` assim:
...
O clawc lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `clawc.json`:
...
**Para agentes embutidos** (em `clawc.json`):
