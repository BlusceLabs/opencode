...
description: Crie ferramentas que o LLM pode chamar no clawc.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do clawc, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.clawc/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/clawc/tools/`.
