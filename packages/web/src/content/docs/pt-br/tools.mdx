...
As ferramentas permitem que o LLM execute ações em sua base de código. O clawc vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no clawc.
...
:::note
Esta ferramenta está disponível apenas quando `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (ou `CLAWC_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor clawc ou quando a variável de ambiente `CLAWC_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o clawc:
