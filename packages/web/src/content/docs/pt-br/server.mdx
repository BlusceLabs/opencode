---
title: Servidor
description: Interaja com o servidor clawc via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `clawc serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente clawc pode usar.
...
```bash
clawc serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `clawc.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
clawc serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CLAWC_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `clawc`, ou defina `CLAWC_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `clawc serve` quanto ao `clawc web`.

```bash
CLAWC_SERVER_PASSWORD=your-password clawc serve
```
...
Quando você executa `clawc`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor clawc para interagir com o clawc programaticamente.
:::

Essa arquitetura permite que o clawc suporte múltiplos clientes e permite que você interaja com o clawc programaticamente.

Você pode executar `clawc serve` para iniciar um servidor autônomo. Se você tiver o TUI do clawc em execução, `clawc serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do clawc [IDE](/docs/ide).
...
O servidor clawc expõe as seguintes APIs.
