...
Создайте `.clawc/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию clawc или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем clawc [config](/docs/config):
...
- Глобальный: `~/.config/clawc/commands/`
- Для каждого проекта: `.clawc/commands/`
...
clawc включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
