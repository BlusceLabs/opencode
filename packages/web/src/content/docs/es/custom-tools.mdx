...
description: Cree herramientas que LLM pueda llamar en clawc.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de clawc como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.clawc/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/clawc/tools/`.
...
```ts title=".clawc/tools/database.ts" {1}
...
```ts title=".clawc/tools/project.ts" {8}
import { tool } from "@clawc/plugin"
...
```python title=".clawc/tools/add.py"
import sys
...
```ts title=".clawc/tools/python-add.ts" {10}
import { tool } from "@clawc/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".clawc/tools/add.py")
