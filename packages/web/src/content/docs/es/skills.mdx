...
Las habilidades del agente permiten a ClawC descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
ClawC busca estas ubicaciones:

- Configuración del proyecto: `.clawc/skills/<name>/SKILL.md`
- Configuración global: `~/.config/clawc/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, ClawC sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.clawc/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.clawc/skills/git-release/SKILL.md` así:
...
ClawC enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `clawc.json`:
...
**Para agentes integrados** (en `clawc.json`):
