---
title: Servidor
description: Interactuar con el servidor clawc a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `clawc serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente clawc puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `clawc.local` |
...
Configure `CLAWC_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `clawc`, o configure `CLAWC_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `clawc serve` como a `clawc web`.
...
Cuando ejecuta `clawc`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor clawc para interactuar con clawc mediante programación.
:::

Esta arquitectura permite que clawc admita múltiples clientes y le permite interactuar con clawc mediante programación.

Puede ejecutar `clawc serve` para iniciar un servidor independiente. Si tienes el
clawc TUI ejecutándose, `clawc serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos ClawC [IDE](/docs/ide).
...
El servidor clawc expone las siguientes API.
