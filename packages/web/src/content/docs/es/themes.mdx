...
Con ClawC puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, ClawC usa nuestro propio tema `clawc`.
...
ClawC viene con varios temas integrados.
...
- Quiere que ClawC coincida con la apariencia de su terminal
...
ClawC admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/clawc/themes/*.json` o `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.clawc/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.clawc/themes/*.json`
