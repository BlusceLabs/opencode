...
Las herramientas permiten que LLM realice acciones en su código base. ClawC viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en ClawC.
...
:::note
Esta herramienta solo está disponible cuando `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (o `CLAWC_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor ClawC o cuando la variable de entorno `CLAWC_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar ClawC:

```bash
CLAWC_ENABLE_EXA=1 clawc
```
