---
title: Complementos
description: Escriba sus propios complementos para extender ClawC.
---

Los complementos le permiten extender ClawC al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de ClawC.
...
- `.clawc/plugins/` - Complementos a nivel de proyecto
- `~/.config/clawc/plugins/` - Complementos globales
...
```json title="clawc.json"
{
  ...
  "plugin": ["clawc-helicone-session", "clawc-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/clawc/node_modules/`.
...
1. Configuración global (`~/.config/clawc/clawc.json`)
2. Configuración del proyecto (`clawc.json`)
3. Directorio global de complementos (`~/.config/clawc/plugins/`)
4. Directorio de complementos del proyecto (`.clawc/plugins/`)
...
ClawC ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".clawc/plugins/my-plugin.ts"
...
```js title=".clawc/plugins/example.js"
...
- `client`: Un cliente SDK clawc para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar clawc.
...
```js title=".clawc/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "clawc"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio ClawC, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que clawc lea archivos `.env`:

```javascript title=".clawc/plugins/env-protection.js"
...
```javascript title=".clawc/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a clawc:

```ts title=".clawc/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que clawc puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para clawc junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://clawc.ai/docs/sdk) para obtener más detalles.
...
```ts title=".clawc/plugins/compaction.ts"
import type { Plugin } from "@clawc/plugin"
...
```ts title=".clawc/plugins/custom-compaction.ts"
import type { Plugin } from "@clawc/plugin"
