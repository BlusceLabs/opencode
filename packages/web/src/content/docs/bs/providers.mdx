---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u ClawC.
---

import config from "../../../../config.mjs"
export const console = config.console

ClawC koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj ClawC konfiguraciji.
...
u `~/.local/share/clawc/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem ClawC
config.
...
ClawC Zen je lista modela koje je obezbedio ClawC tim koji su bili
testirano i potvrđeno da dobro radi sa ClawC. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa ClawC Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite clawc i idite na [clawc.ai/auth](https://clawc.ai/auth).
...
Radi kao i svaki drugi provajder u ClawC i potpuno je opcionalan za korištenje.

---

## ClawC Go

ClawC Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža ClawC tim i koji su testirani i verificirani da dobro rade s ClawC-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `ClawC Go` i idite na [clawc.ai/auth](https://clawc.ai/zen).
...
Da biste koristili Amazon Bedrock s ClawC:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete clawc:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `clawc.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u ClawC.
:::
...
Možete konfigurirati clawc za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi clawc ispravno radio.
   :::
...
Također možete dodati modele kroz svoju clawc konfiguraciju.
...
da pohrani token u clawc auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
ClawC koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate ClawC
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`clawc.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth).

##### Konfiguracija

Prilagodite putem `clawc.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s clawc:
...
Za korištenje Google Vertex AI s ClawC:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće clawc.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem clawca, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session) da automatski prijavite svaki ClawC razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki ClawC razgovor naveden kao zasebna sesija.
...
Možete konfigurirati clawc za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati clawc za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati clawc za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za ClawC. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/clawc) za detalje.
:::
...
Da biste koristili Ollama Cloud s ClawC:
...
3. Kopirajte API ključ za korištenje u ClawC.
...
6. **Važno**: Prije upotrebe modela oblaka u ClawC, morate lokalno povući informacije o modelu:
...
ClawC Zen je lista testiranih i verifikovanih modela koju je obezbedio ClawC tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>ClawC Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **ClawC Zen**.

```txt
   /connect
```

3. Unesite svoj ClawC API ključ.
...
Također možete dodati dodatne modele putem vaše clawc konfiguracije.
...
5. Također ih možete prilagoditi putem vaše clawc konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa ClawCom:
...
Također možete prilagoditi modele kroz svoju clawc konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s clawc-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `clawc.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju ClawC da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `clawc auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite ClawC konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj clawc konfiguraciji.
