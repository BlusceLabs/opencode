...
Možete dodati vanjske alate u ClawC koristeći _Model Context Protocol_, ili MCP. ClawC podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [ClawC Config](https://clawc.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/clawc`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
ClawC automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, ClawC će:
...
Ako server zahtijeva autentifikaciju, ClawC će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `clawc mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, ClawC sigurno čuva tokene u `~/.local/share/clawc/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u ClawC, zajedno s ugrađenim alatima. Možete njima upravljati kroz ClawC konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete ClawC sa vašim Sentry nalogom.
