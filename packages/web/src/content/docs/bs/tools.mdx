...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. ClawC dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u ClawC.
...
:::note
Ovaj alat je dostupan samo kada je `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (ili `CLAWC_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz ClawC provajdera ili kada je varijabla `CLAWC_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju ClawC:
