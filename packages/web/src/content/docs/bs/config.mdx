---
title: Konfiguracija
description: Korištenje ClawC JSON konfiguracije.
---

Možete konfigurirati ClawC koristeći JSON konfiguracijski fajl.

---

## Format

ClawC podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/clawc`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/clawc/clawc.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CLAWC_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`clawc.json` u projektu) - postavke specifične za projekat
5. **`.clawc` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CLAWC_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.clawc` i `~/.config/clawc` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/clawc` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu ClawC konfiguraciju u `~/.config/clawc/clawc.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/clawc/tui.json`.
...
Dodajte `clawc.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se ClawC pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CLAWC_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CLAWC_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.clawc` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.clawc`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`clawc.ai/config.json`**](https://clawc.ai/config.json).

TUI konfiguracija koristi [**`clawc.ai/tui.json`**](https://clawc.ai/tui.json).
...
Koristite `CLAWC_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `clawc.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `clawc serve` i `clawc web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš ClawC server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `clawc.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj ClawC konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, ClawC pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/clawc/agents/` ili `.clawc/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, ClawC će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`clawc run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/clawc/commands/` ili `.clawc/commands/`. [Saznajte više ovdje](/docs/commands).
...
ClawC će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, ClawC **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju ClawC sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.clawc/plugins/` ili `~/.config/clawc/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite ClawC da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
