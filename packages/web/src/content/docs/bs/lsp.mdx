---
title: LSP serveri
description: ClawC se integriše sa vašim LSP serverima.
---

ClawC se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

ClawC dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `CLAWC_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i clawc otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj clawc konfiguraciji.
