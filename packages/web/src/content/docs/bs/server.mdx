---
title: Server
description: Komunicirajte s clawc serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `clawc serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji clawc klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `clawc.local` |
...
Postavite `CLAWC_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `clawc`, ili postavite `CLAWC_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `clawc serve` i za `clawc web`.
...
Kada pokrenete `clawc`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite clawc server za programsku interakciju sa clawc-om.
:::

Ova arhitektura omogucava clawc podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `clawc serve` da startate standalone server. Ako je clawc TUI vec pokrenut, `clawc serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste ClawC [IDE](/docs/ide) pluginovi.
...
clawc server izlaže sljedece API-je.
