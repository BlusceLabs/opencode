...
Agent skills omogucavaju ClawC da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
ClawC pretrazuje ove lokacije:

- Konfiguracija projekta: `.clawc/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/clawc/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, ClawC ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.clawc/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.clawc/skills/git-release/SKILL.md` ovako:
...
ClawC navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `clawc.json`:
...
**Za ugradene agente** (u `clawc.json`):
