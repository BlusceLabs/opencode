...
U ClawC mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, ClawC koristi nasu `clawc` temu.
...
ClawC dolazi sa vise ugradenih tema.
...
- Zele da ClawC odgovara izgledu njihovog terminala
...
ClawC podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/clawc/themes/*.json` ili `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.clawc/themes/*.json`
4. **Trenutni radni direktorij** - `./.clawc/themes/*.json`
