...
Da biste otklonili probleme s ClawC, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\clawc\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `clawc --log-level DEBUG`.

---

## Pohrana

ClawC pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/clawc/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\clawc`
...
ClawC Desktop pokreće lokalni ClawC server (`clawc-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `ClawC` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/clawc/clawc.jsonc` (ili `~/.config/clawc/clawc.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/clawc/clawc.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\clawc\clawc.jsonc`
...
ClawC također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/clawc/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\clawc\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga ClawC mogao ponovo izgraditi.

1. Potpuno zatvorite ClawC Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/clawc`
- **Linux**: obrišite `~/.cache/clawc` (ili pokrenite `rm -rf ~/.cache/clawc`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\clawc`

3. Ponovo pokrenite ClawC Desktop.

---

### Rješavanje problema sa vezom na serveru

ClawC Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `clawc.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CLAWC_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CLAWC_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, ClawC Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa ClawC karakteristikama.

---

### Obavještenja se ne prikazuju

ClawC Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za ClawC u postavkama vašeg OS-a, i
...
1. Zatvorite ClawC Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka ClawC Desktop aplikacije):

- `clawc.settings.dat` (URL zadanog servera za desktop)
- `clawc.global.dat` i `clawc.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `clawc upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `clawc models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\clawc`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. ClawC dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\clawc`

2. Ponovo pokrenite ClawC da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti ClawC da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
ClawC će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
