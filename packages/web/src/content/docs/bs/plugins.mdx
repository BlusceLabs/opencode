---
title: Dodaci
description: Napišite vlastite dodatke za proširenje ClawC.
---

Dodaci vam omogućavaju da proširite ClawC spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja ClawC.
...
- `.clawc/plugins/` - Dodaci na nivou projekta
- `~/.config/clawc/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/clawc/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/clawc/clawc.json`)
2. Konfiguracija projekta (`clawc.json`)
3. Globalni direktorij dodataka (`~/.config/clawc/plugins/`)
4. Direktorij dodataka projekta (`.clawc/plugins/`)
...
ClawC pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: ClawC SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje ClawC.
...
Spriječite clawc da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u clawc:
...
Pomoćnik `tool` kreira prilagođeni alat koji clawc može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za clawc zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://clawc.ai/docs/sdk) za detalje.
