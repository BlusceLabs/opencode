...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. clawc yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
clawc içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (veya `CLAWC_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca clawc provider kullanılırken veya `CLAWC_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

clawc başlatırken etkinleştirmek için:
