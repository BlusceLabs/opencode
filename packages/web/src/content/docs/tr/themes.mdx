...
clawc ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak clawc kendi `clawc` temasını kullanır.
...
clawc birden fazla yerleşik temayla gelir.
...
- clawc'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
clawc, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/clawc/themes/*.json` veya `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.clawc/themes/*.json`
4. **Geçerli çalışma dizini** - `./.clawc/themes/*.json`
