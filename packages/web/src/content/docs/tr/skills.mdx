...
Ajan becerileri, clawc'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
clawc su konumlari tarar:

- Proje konfigurasyonu: `.clawc/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/clawc/skills/<name>/SKILL.md`
...
Proje yerel yollarinda clawc, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.clawc/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.clawc/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
clawc, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`clawc.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`clawc.json` icinde):
