...
description: ClawC, LSP sunucularınızla bütünleşir.
---

ClawC, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

ClawC, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CLAWC_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve clawc bir dosyayı açtığında:
...
LSP sunucularını, clawc yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
