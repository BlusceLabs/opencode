...
_Model Bağlam Protokolü_ veya MCP kullanarak clawc'a harici araçlar ekleyebilirsiniz. clawc hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [clawc Config](https://clawc.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/clawc` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
clawc, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde clawc şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa clawc, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `clawc mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından clawc, belirteçleri `~/.local/share/clawc/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra clawc'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi clawc yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve clawc'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
