---
title: Sağlayıcılar
description: clawc'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

clawc, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı clawc yapılandırmanızda yapılandırın.
...
`~/.local/share/clawc/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları clawc'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
ClawC Zen, clawc ekibi tarafından sağlanan modellerin bir listesidir.
clawc ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz ClawC Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, clawc'u seçin ve [clawc.ai/auth](https://clawc.ai/auth)'ye gidin.
...
clawc'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## ClawC Go

ClawC Go, clawc ile iyi çalıştığı test edilmiş ve doğrulanmış, clawc ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `ClawC Go`'yu seçin ve [clawc.ai/auth](https://clawc.ai/zen) adresine gidin.
...
Amazon Bedrock'u clawc ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   clawc'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `clawc.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi clawc'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
clawc'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   clawc'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   ClawC yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
clawc, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. clawc'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`clawc.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth) ana sayfasında.

##### Yapılandırma

`clawc.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi clawc'la kullanmak için:
...
Google Vertex AI'yi clawc ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları clawc çalıştırılırken ayarlayın.
...
Helicone'un clawc aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her clawc görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her clawc görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
clawc'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla clawc'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla clawc'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini clawc için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/clawc)'a bakın.
:::
...
Ollama Cloud'u clawc ile kullanmak için:
...
3. clawc'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: clawc'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
ClawC Zen, clawc ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>ClawC Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **ClawC Zen**'i arayın.

   ```txt
   /connect
   ```

3. clawc API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   clawc yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca clawc yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi ClawC ile kullanmak için:
...
Modelleri clawc yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı clawc ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `clawc.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları clawc'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `clawc auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için clawc yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, clawc yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
