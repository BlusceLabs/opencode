...
ClawC Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, ClawC'daki diğer sağlayıcılar gibi çalışır. ClawC Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve ClawC'u kullanmak için bunu kullanmanıza gerek yoktur.
...
ClawC Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

ClawC Go, ClawC'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>ClawC Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `ClawC Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye ClawC Go'ya abone olabilir.
:::
...
ClawC Go aşağıdaki limitleri içerir:
...
ClawC yapılandırmanızdaki [model id](/docs/config/#models) formatı `clawc-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `clawc-go/kimi-k2.7-code` kullanmalısınız.
...
ClawC Go'yu şu amaçlarla oluşturduk:
...
4. ClawC ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
