---
title: Eklentiler
description: ClawC'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek ClawC'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya ClawC'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.clawc/plugins/` - Proje düzeyinde eklentiler
- `~/.config/clawc/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/clawc/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/clawc/clawc.json`)
2. Project config (`clawc.json`)
3. Global eklenti dizini (`~/.config/clawc/plugins/`)
4. Proje eklenti dizini (`.clawc/plugins/`)
...
ClawC bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir ClawC SDK istemcisi.
...
ClawC'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
ClawC masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
ClawC'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca ClawC'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, ClawC'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra ClawC'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://clawc.ai/docs/sdk) bakın.
