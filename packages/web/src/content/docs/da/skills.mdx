...
Agentferdigheter lar ClawC oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
ClawC søker etter disse stedene:

- Prosjektkonfigurasjon: `.clawc/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/clawc/skills/<name>/SKILL.md`
...
For prosjektlokale stier går ClawC opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.clawc/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/clawc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.clawc/skills/git-release/SKILL.md` slik:
...
ClawC viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `clawc.json`:
...
**For innebygde agenter** (i `clawc.json`):
