...
Med ClawC kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger ClawC vårt eget `clawc`-tema.
...
ClawC kommer med flere innebygde temaer.
...
- Vil at ClawC skal matche terminalens utseende
...
ClawC støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/clawc/themes/*.json` eller `$XDG_CONFIG_HOME/clawc/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.clawc/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.clawc/themes/*.json`
