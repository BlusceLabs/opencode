---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i ClawC.
---

import config from "../../../../config.mjs"
export const console = config.console

ClawC bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i ClawC-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/clawc/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i ClawC-konfigurationen.
...
ClawC Zen er en liste over modeller leveret af ClawC-teamet, som er blevet testet og verificeret til at fungere godt med ClawC. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med ClawC Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg clawc og gå til [clawc.ai/auth](https://clawc.ai/auth).
...
Det fungerer som alle andre udbydere i ClawC og er helt valgfrit at bruge.

---

## ClawC Go

ClawC Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af ClawC-teamet, som er testet og verificeret til at fungere godt med ClawC.

1. Kør kommandoen `/connect` i TUI, vælg `ClawC Go`, og gå til [clawc.ai/auth](https://clawc.ai/zen).
...
Sådan bruger du Amazon Bedrock med ClawC:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører clawc:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `clawc.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i ClawC understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere clawc til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at clawc skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem clawc-konfigurationen.
...
gemme tokenet i clawc auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
ClawC bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse ClawC
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`clawc.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [clawc-gitlab-auth](https://www.npmjs.com/package/clawc-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `clawc.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med ClawC:
...
Sådan bruger du Google Vertex AI med ClawC:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem clawc, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [clawc-helicone-session](https://github.com/H2Shami/clawc-helicone-session) plugin for automatisk at logge hver ClawC-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver ClawC-samtale opført som en separat session.
...
Du kan konfigurere clawc til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere clawc til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere clawc til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for ClawC. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/clawc) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med ClawC:
...
3. Kopier API-nøglen til brug i ClawC.
...
6. **Vigtigt**: Før du bruger skymodeller i ClawC, skal du hente modelinformationen lokalt:
...
ClawC Zen er en liste over testede og verificerede modeller leveret af ClawC-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>ClawC Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **ClawC Zen**.

   ```txt
   /connect
   ```

3. Indtast ClawC API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem clawc-konfigurationen.
...
5. Du kan også tilpasse dem gennem clawc-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med ClawC:
...
Du kan også tilpasse modeller gennem clawc-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med clawc. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `clawc.json`-filen i projektkataloget:
...
`limit`-felterne lader ClawC forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `clawc auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek clawc-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i clawc-konfigurationen.
