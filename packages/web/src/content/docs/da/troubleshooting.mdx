...
For at fejlfinde problemer med ClawC, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\clawc\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `clawc --log-level DEBUG`.

---

## Lagring

clawc gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/clawc/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\clawc`
...
ClawC Desktop kører en lokal ClawC-server (`clawc-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `ClawC`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/clawc/clawc.jsonc` (eller `~/.config/clawc/clawc.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/clawc/clawc.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\clawc\clawc.jsonc`
...
ClawC kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/clawc/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\clawc\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så ClawC kan genopbygge den.

1. Afslut ClawC Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/clawc`
- **Linux**: slet `~/.cache/clawc` (eller kør `rm -rf ~/.cache/clawc`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\clawc`

3. Start ClawC Desktop på ny.

---

### Løs problemer med serverforbindelse

ClawC Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `clawc.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CLAWC_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CLAWC_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver ClawC Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i ClawC.

---

### Meddelelser vises ikke

ClawC Desktop viser kun systemvarsler når:

- varsler er aktiveret for ClawC i dine OS-indstillinger, og
...
1. Afslut ClawC Desktop.
2. Find og slet disse filer (de findes i ClawC Desktop-appens datamappe):

- `clawc.settings.dat` (skrivebordsstandardserver URL)
- `clawc.global.dat` og `clawc.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med ClawC:
...
For at finde ud af hvilke modeller du har adgang til, kør `clawc models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\clawc`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. clawc installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\clawc`

2. Start clawc på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge clawc til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
clawc vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
