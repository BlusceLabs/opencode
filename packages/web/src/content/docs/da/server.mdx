---
title: Server
description: Interager med clawc-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `clawc serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en clawc-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `clawc.local` |
...
Indstil `CLAWC_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `clawc`, eller indstil `CLAWC_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `clawc serve` og `clawc web`.
...
Når du kører `clawc` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug clawc-serveren til at interagere med clawc programmatisk.
:::

Denne arkitektur lader clawc understøtte flere klienter og lader dig interagere med clawc programmatisk.

Du kan køre `clawc serve` for at starte en selvstændig server. Hvis du har
clawc TUI kørende, vil `clawc serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af ClawC [IDE](/docs/ide) plugins.
...
ClawC-serveren viser følgende API'er.
