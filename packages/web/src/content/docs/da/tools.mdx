...
Her er alle de innebygde verktøyene som er tilgængelige i ClawC.
...
:::note
Dette verktøyet er bare tilgængelig når `CLAWC_EXPERIMENTAL_LSP_TOOL=true` (eller `CLAWC_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger ClawC-leverandøren eller når miljøvariabelen `CLAWC_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
