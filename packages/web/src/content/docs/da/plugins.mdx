---
title: Plugins
description: Skriv dine egne plugins for at udvide ClawC.
---

Plugins giver dig mulighed for at udvide ClawC ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre ClawCs standardadfærd.
...
- `.clawc/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/clawc/node_modules/`.
...
1. Global konfiguration (`~/.config/clawc/clawc.json`)
2. Projektkonfiguration (`clawc.json`)
3. Global plugin-mappe (`~/.config/clawc/plugins/`)
4. Projekt plugin bibliotek (`.clawc/plugins/`)
...
ClawC kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En clawc SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide clawc.
...
:::note
Hvis du bruger ClawC desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå clawc i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til clawc:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som clawc kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for clawc sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://clawc.ai/docs/sdk) for detaljer.
