...
> current private `@clawc/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@clawc/ai`
...
- Secondary audience: ClawC and other durable agent runtimes
...
import { LLM } from "@clawc/ai"
import { OpenAI } from "@clawc/ai/providers/openai"
...
ClawC and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@clawc/ai/promise"
import { OpenAI } from "@clawc/ai/providers/openai"
...
```ts
import { LLMSchema } from "@clawc/ai/schema"
...
```ts
import { Provider, Protocol } from "@clawc/ai/provider"
...
@clawc/ai/providers/openai
@clawc/ai/providers/anthropic
@clawc/ai/providers/google
...
```
...
ClawC should migrate to `generateTurn` / `streamTurn`, preserving its durable
