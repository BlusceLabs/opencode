...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = lib.optionalString stdenv.isLinux ''
    ...
    for file in "''${FILES[@]}"; do
      substituteInPlace $BASE_PATH/$file \
        --replace-fail "process.resourcesPath" "'$out/opt/clawc-desktop/resources'"
    done
  '';
  ...
  installPhase =
    ...
    + lib.optionalString stdenv.hostPlatform.isLinux ''
      ...
      cp -r dist/linux*-unpacked/{resources,LICENSE*} $out/opt/clawc-desktop
      makeWrapper ${lib.getExe electron} $out/bin/clawc-desktop \
        --inherit-argv0 \
        --set ELECTRON_FORCE_IS_PACKAGED 1 \
        --add-flags $out/opt/clawc-desktop/resources/app.asar \
